...
gmsh.initialize()
gdim = 2
mesh, ft, inlet_marker, wall_marker, outlet_marker, obstacle_marker = mesh_init.create_mesh(gdim, False)
...
drag = form(2/0.1 * (vs_n * inner(grad(u_t*U_n), n) * n[1] - p_ * n[0]) * dObs)
lift = form(-2/0.1 * (vs_n * inner(grad(u_t*U_n), n) * n[0] + p_ * n[1]) * dObs)
if mesh.comm.rank == 0:
...
with dolfinx.io.VTXWriter(MPI.COMM_WORLD, np_path + f"/{experiment_number}_{str(vs_n)}_pressure.bp", [p_],
                          engine="BP4") as vtx:
    vtx.write(0.0)
with dolfinx.io.VTXWriter(MPI.COMM_WORLD, np_path + f"/{experiment_number}_{str(vs_n)}_u.bp", [u_], engine="BP4") as vtx:
    vtx.write(0.0)
# ---------------------------------------------------------------------------------------------------------------------
